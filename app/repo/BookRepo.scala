...
class BookRepo @Inject() (
  ...
  def updateBook(book: BookPatch): Future[Either[String, Book]] = {
    ...
    db.run(finder.result.headOption).flatMap {
      case None =>
        Future.successful(
          Left("Book Not Found")
        ) // If no book is found with that id
      case Some(existingBook) =>
        ...
        if (
          existingBook == updatedBook.copy(updated_at = existingBook.updated_at)
        ) {
          Future.successful(
            Left("No changes are made")
          ) // if the previous book and the updated is same, no changes are made is returned
        } else {
  ...
  def getBookFine(bookId: Long): Future[Int] = {
    db.run(books.filter(_.id === bookId).map(_.fine).result.headOption)
      .flatMap {
        case Some(fine) =>
          Future.successful(
            fine
          ) // Fetch the fine_per_day value from the DB for the book
        case None =>
          Future.failed(
            new NoSuchElementException(s"Book with ID $bookId not found")
          )
      }
  }
  ...
  def decreaseStock(bookId: Long): Future[Int] = {
    val query =
      books.filter(_.id === bookId).map(_.stock).result.headOption.flatMap {
      ...
      books.filter(_.id === bookId).map(_.stock).result.headOption.flatMap {
        case Some(stock) =>
          books
            .filter(_.id === bookId)
            .map(_.stock)
            .update(stock + 1) // updates the stock +1 on return
        case None =>
          DBIO.successful(0)
      }
    db.run(query)
