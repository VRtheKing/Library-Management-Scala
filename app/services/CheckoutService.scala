...
class CheckoutService @Inject() (
  ...
  def createCheckout(checkout: Checkout): Future[Either[String, Int]] = {
    val queryTransaction: DBIO[Either[String, Int]] = for {
      ...
      result <- user match {
        ...
        case Some(_) =>
          for {
            stockAvl <- bookRepo.books
              .filter(_.id === checkout.bookId)
              .map(_.stock)
              .forUpdate
              .result
              .headOption
            outcome <- stockAvl match {
              ...
              case Some(stock) if stock <= 0 =>
                DBIO.successful(Left("Book out of stock"))
    ...
    checkoutRepo.findById(update.id).flatMap {
      ...
      case Some(existingCheckout) =>
        ...
        val resetOldStockF = if (updatedBookId != existingCheckout.bookId) {
          bookRepo.increaseStock(
            existingCheckout.bookId
          ) // rollback old book's stock
    ...
    status match {
      ...
      case "PENDING" =>
        checkoutRepo.findPendingCheckouts(
          LocalDate.now()
        ) // Gets the checkouts that are pending
      case "OVERDUE" =>
        checkoutRepo.findOverdueCheckouts(
          LocalDate.now()
        ) // Gets the checkouts that are overdue
      case _ =>
        Future.failed(new IllegalArgumentException("Invalid Query Option"))
    }
  }
    ...
    checkoutRepo.findById(checkoutId).flatMap {
      case Some(checkout) =>
        if (checkout.returned) {
          Future.successful(
            Left("Book already returned")
          ) // If a duplicate return request is made
        } else {
      ...
      case None =>
        ...
        ) // Returns Left if no checkout is found on the id
    }
  }
  ...
  def calculateFine(checkoutId: Long): Future[Int] = {
    checkoutRepo.findById(checkoutId).flatMap {
      case Some(checkout) =>
        val today = checkout.returnDate.getOrElse(LocalDate.now())
        bookRepo.getBookFine(checkout.bookId).flatMap {
          fine_amt => // Gets the fine amount of that book for a day from DB
            ...
            checkoutRepo
              .calculateFine(checkoutId, Some(fineAmount))
              .map(_ => fineAmount) // Updates the Fine to the DB
        }
      case None =>
        Future.successful(0)
    }
