...
class NotificationService @Inject() (notificationRepo: NotificationRepo)(implicit
    ec: ExecutionContext
) {
  def notify(message: String): Future[Int] = {
    println(s"[NOTIFICATION] -> $message") // Console Notification
    notificationRepo.logNotification(
      message
    ) // Logs the notification to the console and DB
  }
