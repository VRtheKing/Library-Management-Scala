...
class BookModel(tag: Tag) extends Table[Book](tag, "books") {
  ...
  def * = (id.?, title, author, isbn, stock, updated_at.?, fine)
    .mapTo[Book] // Projection Mapping
}

object Book {
  implicit val bookFormat: OFormat[Book] = Json.format[Book]
  implicit val bookPatchFormat: OFormat[BookPatch] =
    Json.format[BookPatch] // Book patch serialization
}
