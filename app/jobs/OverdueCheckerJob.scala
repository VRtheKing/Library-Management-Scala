    ...
    checkoutService.findOverdueCheckouts().foreach { overdueCheckouts =>
      overdueCheckouts.foreach { checkout => // finds all the overdue checkouts
        userService.getUsername(checkout.userId).foreach {
          case Some(username) =>
            val msg =
              s"User $username (Id: ${checkout.userId}) has an overdue book (Book ID: ${checkout.bookId}). Due: ${checkout.dueDate}"
            notificationService.notify(msg) // Sends out the notification
          case None =>
            val msg =
              s"UserId: ${checkout.userId} (username not found) has an overdue book (Book ID: ${checkout.bookId}). Due: ${checkout.dueDate}"
            notificationService.notify(msg) // Sends out the notification
        }
        checkout.id.foreach(id => checkoutService.calculateFine(id) // Updates the fine to the DB
        )
      }
    }
  }
