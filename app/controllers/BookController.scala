...
class BookController @Inject()(
  ...
  def createBook: Action[JsValue] = jwtAction.async(parse.json) { request =>
    if (hasRole(List(1, 2))(request)) {
      ...
      Future.successful(Forbidden(Json.obj("status" -> "You do not have permission to create a book")))
    }
  }
  ...
  def updateBook(): Action[JsValue] = jwtAction.async(parse.json) { request =>
    if (hasRole(List(1, 2))(request)) {
      ...
      Future.successful(Forbidden(Json.obj("status" -> "You do not have permission to update the book")))
    }
  }
  ...
  def listBooks: Action[AnyContent] = jwtAction.async { request =>
    if (hasRole(List(1,2,3))(request)) {
      ...
      Future.successful(Forbidden(Json.obj("status" -> "You do not have permission to view all books")))
    }
  }
